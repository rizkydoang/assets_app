...
class Profit_center extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->template->load('overview', 'report/profitcenter/v_profitcenter', $data);
	}
	// {
	// 		if($this->input->post('date_from') != null || $this->input->post('date_to') != null){
	// 			if($this->input->post('division') != null){
	// 				$data['profit_center'] = $this->m_allreport->profit_center($this->input->post('date_from'), $this->input->post('date_to'), $this->input->post('division'));
	// 				$data['parent_division']= $this->m_division->listDivision();
	// 				$this->template->load('overview', 'report/profitcenter/v_profitcenter', $data);
	// 			}else{
	// 				$data['profit_center'] = $this->m_allreport->profit_center($this->input->post('date_from'), $this->input->post('date_to'));
	// 				$data['parent_division']= $this->m_division->listDivision();
	// 				$this->template->load('overview', 'report/profitcenter/v_profitcenter', $data);
	// 			}
	// 		}else{
	// 			if($this->input->post('division') != null){
	// 				$data['profit_center'] = $this->m_allreport->profit_center(null,null, $this->input->post('division'));
	// 				$data['parent_division']= $this->m_division->listDivision();
	// 				$this->template->load('overview', 'report/profitcenter/v_profitcenter', $data);
	// 			}else{
	// 				$data['profit_center'] = $this->m_allreport->profit_center();
	// 				$data['parent_division']= $this->m_division->listDivision();
	// 				$this->template->load('overview', 'report/profitcenter/v_profitcenter', $data);
	// 		}	
	// 	}
	...
	public function getProfitCenter()
	{
		...
		$list = $this->m_allreport->profit_center($dateFrom, $dateTo, $division);
		$data = array();
		$no = 1;
	...
	public function profitCenterExportExcel(){
		// $data['profitCenter'] = $this->m_allreport->profit_center();
		...
		$sheet->setCellValue('I1','Profit Center');
		...
		foreach ($list as $dataExcel) {
			...
			$sheet->setCellValue('I'.$baris,$dataExcel->product_profit_center);
		...
		$filename = 'Profit_Report.xlsx'; //save our workbook as this file name
